...
class ActorNet(nn.Module):
    ...
    def forward(self, x):
        if self.__is_discr_state:
            ...
            mean = torch.sigmoid(self.output_layer(hidde_x))
            std = torch.nn.Parameter(0.2 * torch.ones(1, dtype=torch.float32)).to(device)
            return mean, std
        else:
            conti_x = x[:, 0:2]
            mean =  torch.sigmoid(self.layers(conti_x))
            std = torch.nn.Parameter(0.2 * torch.ones(1, dtype=torch.float32)).to(device)
            return mean, std
...
class PPO_C(Agent):
    def __init__(self, config, agent_config, is_eval) -> None:
        super(PPO_C, self).__init__(config, agent_config, is_eval)
        ...
        self.__actor_net = ActorNet(self._state_size, out_size=1, hidde_size=self._hidde_size,
                                    discr_state_info=discr_state_info).to(device)
    ...
    def cal_hold_time(self, snapshot):
        ...
        track_equal_rewar, track_inten_rewar, track_rewar = self.__event_handl.add_event(
            snapshot, action, hold_time, self._w, log_prob)
    ...
    def act(self, state):
        ...
        with torch.no_grad():
            mean, std = self.__actor_net(state)
    ...
    def learn(self):
        ...
        a = torch.tensor(a_list, dtype=torch.float32).to(device).view(-1, 1)
        ...
        with torch.no_grad():
            old_means, old_stds = self.__actor_net(s)
        old_dist = torch.distributions.Normal(old_means, old_stds)
        old_log_probs = old_dist.log_prob(a).detach()

        for _ in range(self.__K_epoch):
            new_means, new_stds = self.__actor_net(s)
