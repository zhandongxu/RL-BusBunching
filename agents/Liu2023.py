...
class Liu(Agent):
    def __init__(self, config, agent_config, is_eval) -> None:
        super(Liu, self).__init__(config, agent_config, is_eval)

        self.__epsilon0 = agent_config["epsilon0"]
        self.__epsilon = 1
        self.__sigma = agent_config["sigma"]
        ...
        self.__critic_net = CriticNet(self._state_size+2, self._action_size, discr_state_info, self._hidde_size).to(device)
    ...
    def cal_hold_time(self, snapshot):
        ...
        state.append(snapshot.bus_load)
        state.append(snapshot.pax_demand)
    ...
    def act(self, state):
        ...
        state = torch.tensor(state, dtype=torch.float32).reshape(1, 5).to(device)
        a = torch.argmax(self.__critic_net(state))
        return a.item()

    def schedule_hyperparameters(self, t):
        self.__epsilon = self.__epsilon0 / (1+ (t**2 / (self.__sigma + t)))
