...
from network import make_seq, CriticNetwork
...
class DQN(Agent):
    def __init__(self, config, agent_config, is_eval) -> None:
        super(DQN, self).__init__(config, agent_config, is_eval)
    ...
    def learn(self):
        ...
        states, actions, rewards, next_states= self.__event_handl.sample_transition(self._batch_size)
        ...
        with torch.no_grad():
            max_next_q = torch.max(self.__critic_net(n_s), 1).values.unsqueeze(1)
